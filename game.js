  ...
  drops.forEach((drop, index) => {
    ...
    if (!drop.caught && drop.y > canvas.height) {
      if (!drop.bonus && !drop.slowDown) {
        missedDrops++;
        if (missedDrops >= maxMisses) endGame();
      }
...
function startGame() {
  ...
  lastSpawn = performance.now(); // ✅ fix: reset spawn timing
}
